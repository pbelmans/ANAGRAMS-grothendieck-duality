...
\begin{theorem}[Riemann--Roch]
  ...
  Let~$\mathcal{L}$ be an invertible sheaf of degree~$d$ on~$C$. Then\fixthis{nice definition of $\Omega^1$}
  \begin{equation}
...
Let~$M$ be the set of meromorphic differentials, which is a one-dimensional~$k(C)$\dash vectorspace\expand{why?}. For every point~$p\in C$ there is a \emph{residue map}
\begin{equation}
  \res_p\colon M\to k,
\end{equation}
...
\begin{proposition}
  Let~$H$ be an ample divisor on a surface~$S$. Then there exists an integer~$n_0$ (in fact, it is~$\mathrm{K}_S\cdot H$) such that, if~$D$ is a divisor for which~$D\cdot H>0$, then~$\HH^2(S,\mathcal{L}(D))=0$.
\end{proposition}
