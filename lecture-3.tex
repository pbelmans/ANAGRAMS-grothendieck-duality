...
\end{example}
\begin{example}
  Another well-known left exact functor is~$\Hom_A(M,-)$ (let's take~$\mathcal{A}=A\mhyphen\mathrm{Mod}$ for~$A$ a commutative ring and~$M$ an~$A$\dash module), whose right-derived functors are the Ext-functors.
\end{example}
\begin{example}
