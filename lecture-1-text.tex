...
\begin{lemma}[Cheap Riemann--Roch]
  ...
  \begin{proof}
    The invertible sheaf~$\mathcal{L}$ can always be written as~$\mathcal{O}_C(p_1+\dotso+p_a-q_1-\dotso-q_b)$ for~$p_i$ and~$q_j$ points on~$C$, because in this case the Picard group (the group of invertible sheaves) is isomorphic to the group of Cartier divisors (which agree with Weil divisors, the most down-to-earth version we use here). We get~$a-b=\deg(\mathcal{L})$.
...
\begin{lemma}
  ...
  \begin{proof}
    ...
    \begin{equation}
      \begin{aligned}
        &\dim_k\mathrm{J}(D-D_n)\\
        &\qquad=\dim_k\mathrm{I}(D-D_n) & \text{definition} \\
        &\qquad=\hh^1(C,\mathcal{O}_C(D-D_n)) & \text{\cref{proposition:HH1}} \\
        &\qquad=\hh^0(C,\mathcal{O}_C(D-D_n))-(d-n)+\text{constant} & \text{cheap Riemann--Roch} \\
        &\qquad=n+\text{constant}' & n\gg 0.
      \end{aligned}
    \end{equation}
